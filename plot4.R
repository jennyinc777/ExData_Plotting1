...
par(mfrow = c(2, 2))
plot(epcFinal$`Date & Time`,epcFinal$Global_active_power, type = "l", xlab = "", ylab = "Global Active Power")
plot(epcFinal$`Date & Time`,epcFinal$Voltage, type = "l", xlab = "datetime", ylab = "Voltage")
plot(epcFinal$`Date & Time`, epcFinal$Sub_metering_1, type = "l", xlab = "", ylab = "Energy sub metering")
lines(epcFinal$`Date & Time`, epcFinal$Sub_metering_2, type = "l", col = "red")
lines(epcFinal$`Date & Time`, epcFinal$Sub_metering_3, type = "l", col = "blue")
legend("topright", lty = c(1,1), bty = "n", y.intersp = .7, col = c("black", "red", "blue"), legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"))
plot(epcFinal$`Date & Time`,epcFinal$Global_reactive_power, type = "l", xlab = "datetime", ylab = "Global_reactive_power")
